# Okê Sports - Contexto do Projeto

**Plataforma SaaS para gestão de inscrições e eventos esportivos**

**Status:** 📋 Planejamento completo | 🚀 Pronto para iniciar desenvolvimento
**Última atualização:** 31/10/2025
**Versão do planejamento:** 2.0
...
### O que é?
Okê Sports é uma **plataforma marketplace bilateral** que conecta organizadores de eventos esportivos com participantes (atletas), facilitando inscrições, pagamentos e gestão de eventos.
...
- **Mais barato:** Taxa de 10% vs 12% da concorrência (Ticket Sports)
- **Repasse rápido:** PIX em D+0, sem taxa de repasse

### Etimologia do Nome
**Okê** = "Porta" ou "Entrada" em Tupi-Guarani → perfeito para plataforma de acesso a eventos

---

## 💰 MODELO DE NEGÓCIO

### Monetização (Marketplace Bilateral - Taxa Única 10%)
```
┌─────────────────────────────────────────────┐
│ PARTICIPANTE                                │
│ Inscrição: R$ 80,00                         │
│ Taxa Okê Sports: R$ 8,00 (10%)              │
│ TOTAL PAGA: R$ 88,00                        │
└─────────────────────────────────────────────┘
              ↓ (Mercado Pago Split)
┌─────────────────────────────────────────────┐
│ SPLIT AUTOMÁTICO                            │
│ • Organizador: R$ 79,13 (D+0 para PIX)     │
│ • Okê Sports: R$ 8,00                       │
│ • Taxa Gateway: R$ 0,87 (paga pelo org)    │
└─────────────────────────────────────────────┘
```

**Modelo simplificado:**
- ✅ **Taxa única:** 10% sobre valor da inscrição
- ✅ **Sem mensalidade:** 100% gratuito para organizador
- ✅ **Sem limites:** Eventos e inscrições ilimitadas
- ✅ **Repasse D+0:** Dinheiro cai direto na conta do organizador via PIX
...
### Backend
- **Runtime:** Next.js API Routes (MVP) → Node.js/Express (escala)
- **Linguagem:** TypeScript
- **Banco de dados:** PostgreSQL 14+ (Neon/Supabase para MVP)
- **ORM:** Prisma
- **Cache:** Redis (Upstash)
- **Queue:** Bull (se necessário)
...
│  • /api/registrations/* (inscrições)            │
│  • /api/payments/* (checkout, status)           │
│  • /api/webhooks/mercadopago                    │
│  • /api/organizers/* (dashboard, stats)         │
└─────────────────────────────────────────────────┘
                      ↓
┌─────────────────────────────────────────────────┐
...
│  • PaymentService (Mercado Pago SDK)            │
│  • RegistrationService (inscrições)             │
│  • EmailService (Resend)                        │
│  • UploadService (S3/R2)                        │
└─────────────────────────────────────────────────┘
                      ↓
...
│         PostgreSQL Database                      │
│  • Tabelas: User, Event, Registration,          │
│    Batch, Coupon, Payment, OAuthToken           │
└─────────────────────────────────────────────────┘
                      ↓
┌─────────────────────────────────────────────────┐
│        SERVIÇOS EXTERNOS (Integrações)          │
│  • Mercado Pago (OAuth + Payments + Webhooks)   │
...
model Batch {
  ...
  // Período (se type = DATE)
  startDate   DateTime?
  endDate     DateTime?

  // Volume (se type = VOLUME)
  maxSales    Int?      // Máximo de vendas neste lote
...
```
┌─────────────────────────────────────────────────────┐
│ 1. ORGANIZADOR CLICA "CONECTAR MERCADO PAGO"       │
└─────────────────────────────────────────────────────┘
                      ↓
┌─────────────────────────────────────────────────────┐
│ 2. GERAR STATE (CSRF Token)                        │
│    state = crypto.randomBytes(32).toString('hex')  │
│    Salvar em session/Redis com TTL 10min           │
└─────────────────────────────────────────────────────┘
                      ↓
┌─────────────────────────────────────────────────────┐
│ 3. REDIRECT PARA MERCADO PAGO OAuth                │
...
│      - platform_id=mp                               │
│      - state={state}                                │
│      - redirect_uri={APP_URL}/api/auth/mp/callback │
└─────────────────────────────────────────────────────┘
                      ↓
┌─────────────────────────────────────────────────────┐
│ 4. USUÁRIO AUTORIZA NO MERCADO PAGO                │
│    (Login, permissões, aceitar)                     │
└─────────────────────────────────────────────────────┘
                      ↓
┌─────────────────────────────────────────────────────┐
│ 5. CALLBACK: /api/auth/mp/callback                 │
│    Recebe: ?code={auth_code}&state={state}         │
└─────────────────────────────────────────────────────┘
                      ↓
┌─────────────────────────────────────────────────────┐
│ 6. VALIDAR STATE                                    │
│    if (state !== sessionState) throw Error         │
└─────────────────────────────────────────────────────┘
                      ↓
┌─────────────────────────────────────────────────────┐
...
└─────────────────────────────────────────────────────┘
                      ↓
┌─────────────────────────────────────────────────────┐
│ 9. CRIPTOGRAFAR E SALVAR NO BANCO                  │
│    User.update({                                    │
│      mpConnected: true,                             │
│      mpUserId: user_id,                             │
│      mpAccessToken: encrypt(access_token),          │
│      mpRefreshToken: encrypt(refresh_token),        │
│      mpPublicKey: public_key,                       │
│      mpTokenExpiresAt: now + expires_in             │
│    })                                               │
└─────────────────────────────────────────────────────┘
                      ↓
┌─────────────────────────────────────────────────────┐
...
│    Body: {                                         │
│      eventId,                                      │
│      items: [{participantData, modalityId, ...}]   │
│    }                                               │
└────────────────────────────────────────────────────┘
                      ↓
┌────────────────────────────────────────────────────┐
...
│    • Vagas disponíveis?                            │
│    • Cupons válidos?                               │
│    • Organizador tem MP conectado?                 │
└────────────────────────────────────────────────────┘
                      ↓
┌────────────────────────────────────────────────────┐
│ 4. CRIAR REGISTRATIONS (status: PENDING)          │
│    Salvar no banco com paymentStatus: PENDING     │
└────────────────────────────────────────────────────┘
                      ↓
┌────────────────────────────────────────────────────┐
│ 5. DESCRIPTOGRAFAR mpAccessToken DO ORGANIZADOR   │
│    token = decrypt(organizer.mpAccessToken)       │
└────────────────────────────────────────────────────┘
                      ↓
┌────────────────────────────────────────────────────┐
│ 6. CRIAR PREFERENCE NO MERCADO PAGO               │
│    POST https://api.mercadopago.com/checkout/      │
│         preferences                                │
│    Headers: Authorization: Bearer {access_token}   │
│    Body: {                                         │
│      items: [{                                     │
│        title: "Inscrição - Corrida SP",            │
│        quantity: 1,                                │
│        unit_price: 216.00                          │
│      }],                                           │
│      marketplace_fee: 21.60, // 10% split          │
│      notification_url: "{APP_URL}/api/webhooks/mp",│
│      back_urls: {...},                             │
│      external_reference: registrationIds.join(',') │
│    }                                               │
└────────────────────────────────────────────────────┘
                      ↓
┌────────────────────────────────────────────────────┐
│ 7. RECEBER PREFERENCE ID                          │
│    Response: {                                     │
...
└────────────────────────────────────────────────────┘
                      ↓
┌────────────────────────────────────────────────────┐
│ 8. REDIRECT ATLETA PARA MERCADO PAGO              │
│    window.location = preference.init_point         │
└────────────────────────────────────────────────────┘
                      ↓
┌────────────────────────────────────────────────────┐
│ 9. ATLETA PAGA (PIX, Cartão, Boleto)             │
└────────────────────────────────────────────────────┘
                      ↓
┌────────────────────────────────────────────────────┐
│ 10. MERCADO PAGO PROCESSA PAGAMENTO               │
│     • PIX: aprovação instantânea                   │
│     • Cartão: análise antifraude (~2min)           │
└────────────────────────────────────────────────────┘
                      ↓
┌────────────────────────────────────────────────────┐
│ 11. WEBHOOK: POST /api/webhooks/mercadopago       │
│     Body: {                                        │
...
│    Verificar x-signature com MP_WEBHOOK_SECRET    │
│    if (!valid) return 401                          │
└────────────────────────────────────────────────────┘
                      ↓
┌────────────────────────────────────────────────────┐
│ 3. VERIFICAR IDEMPOTÊNCIA                         │
│    if (processedWebhooks.has(x-request-id))        │
│      return 200 // Já processado                   │
└────────────────────────────────────────────────────┘
                      ↓
┌────────────────────────────────────────────────────┐
│ 4. BUSCAR DETALHES DO PAGAMENTO                   │
│    GET https://api.mercadopago.com/v1/payments/{id}│
│    Headers: Authorization: Bearer {access_token}   │
└────────────────────────────────────────────────────┘
                      ↓
┌────────────────────────────────────────────────────┐
│ 5. RECEBER DADOS DO PAGAMENTO                     │
│    Response: {                                     │
│      id,                                           │
│      status: "approved",                           │
│      status_detail,                                │
│      external_reference: "reg1,reg2,reg3",         │
│      transaction_amount: 216.00,                   │
│      payment_method_id: "pix"                      │
│    }                                               │
└────────────────────────────────────────────────────┘
                      ↓
┌────────────────────────────────────────────────────┐
│ 6. EXTRAIR REGISTRATION IDs                       │
│    registrationIds = external_reference.split(',') │
└────────────────────────────────────────────────────┘
                      ↓
┌────────────────────────────────────────────────────┐
│ 7. ATUALIZAR REGISTRATIONS NO BANCO               │
...
└────────────────────────────────────────────────────┘
                      ↓
┌────────────────────────────────────────────────────┐
│ 8. ATUALIZAR SLOTS DA MODALIDADE                  │
│    Modality.increment({ soldSlots: +2 })           │
└────────────────────────────────────────────────────┘
                      ↓
┌────────────────────────────────────────────────────┐
│ 9. ENVIAR EMAILS DE CONFIRMAÇÃO                   │
│    Para cada registration:                         │
│      - Email para participante                     │
│      - Email para organizador (resumo)             │
└────────────────────────────────────────────────────┘
                      ↓
┌────────────────────────────────────────────────────┐
│ 10. MARCAR WEBHOOK COMO PROCESSADO                │
│     Cache.set(x-request-id, true, TTL: 24h)        │
└────────────────────────────────────────────────────┘
                      ↓
┌────────────────────────────────────────────────────┐
│ 11. RETORNAR 200 OK                                │
│     (MP requer resposta rápida < 5s)               │
└────────────────────────────────────────────────────┘
```
...
- **Sanitização:** Zod validation em todas inputs
- **LGPD:** desde M1 (termos, privacidade, consentimento)
- **Webhook Signature:** Validação obrigatória de assinatura MP
...
- [x] Modelo de negócio definido e validado
...
- [x] **M0: Validação de Mercado** - Considerado validado
- [x] **M1.1: Setup do Projeto** - Next.js 14 + Prisma + Tailwind
- [x] **M1.2: Autenticação (JWT)** - Login/Register organizador + middleware + hash de senha
- [x] **M1.3: OAuth Mercado Pago (organizador)** - Fluxo básico de conexão e armazenamento de tokens
- [x] **M2: CRUD inicial de Eventos + Modalidades** - Criação/listagem com slug único e modalidades básicas

### 🚧 Em Andamento (Backend MVP)
- [ ] Ajustes finos em pagamentos:
  - Implementar `marketplace_fee` e split efetivo da taxa de 10% nas preferences do Mercado Pago
  - Suporte a múltiplas inscrições no mesmo checkout (carrinho)
- [ ] Segurança do webhook Mercado Pago:
  - Validação de assinatura (`x-signature` + `MP_WEBHOOK_SECRET`)
  - Idempotência (não processar o mesmo `x-request-id` 2x)
- [ ] Refinos no fluxo do atleta:
  - Fluxo completo: página pública → login/cadastro → inscrição → pagamento → telas de sucesso/falha/pendente

### 📅 Próximos Passos (Super MVP)
1. Consolidar backend MVP de pagamentos
   - Finalizar marketplace fee e split payment automático
   - Implementar segurança (assinatura + idempotência) no webhook

2. Fechar fluxo completo do atleta (checkout simples)
   - Inscrição individual por modalidade com PIX/cartão
   - Telas de sucesso/falha/pendente ligadas ao webhook

3. Refinar dashboard do organizador
   - Edição/publicação de eventos
   - Gestão de modalidades
   - Link público pronto para compartilhamento
...
│   ├── (dashboard)/       # Grupo de rotas: área do organizador
...
MP_CLIENT_ID="..."
MP_CLIENT_SECRET="..."
MP_ACCESS_TOKEN="..." # Token da Okê Sports (receber split)
MP_PUBLIC_KEY="..."
...
- [ ] PIX + Cartão 100% funcionais
- [ ] Taxa de sucesso pagamento > 95%

### Checkpoint 2 (Mês 3 - Crescimento)
...
**Decisão:** Organizador conecta sua própria conta do gateway (OAuth)
**Vantagem:**
- ✅ Zero risco financeiro para plataforma
- ✅ Não somos instituição de pagamento
- ✅ Sem compliance bancário complexo
- ✅ Organizador recebe dinheiro direto
- ✅ Mais confiança do organizador

**Trade-off:**
- ❌ Requer onboarding do organizador (conectar MP)
- ❌ Dependência do gateway (mas baixa - split nativo)

### 2. Por que Mercado Pago e não Stripe?
**Decisão:** Mercado Pago como gateway principal
**Vantagem:**
- ✅ Split payment nativo
- ✅ PIX com taxa de 0,99% (Stripe: 1%)
- ✅ Reconhecimento de marca no Brasil
- ✅ Suporte em português
- ✅ Documentação completa

**Manter Stripe como backup para futuro**

### 3. Por que Next.js e não separar front/back?
**Decisão:** Monolito Next.js no MVP
**Vantagem:**
...
**Futuro:** Separar em microsserviços se necessário (escala)

### 4. Por que PIX + Cartão no MVP?
**Decisão:** Incluir cartão de crédito desde o início (não só PIX)
**Vantagem:**
- ✅ Eventos esportivos custam R$ 80-300
- ✅ Público quer parcelar
- ✅ PIX sozinho limita vendas
- ✅ Mais credibilidade profissional

**Essential para mostrar valor real**

### 5. Por que Freemium e não só Split?
**Decisão:** Modelo freemium com planos
**Vantagem:**
- ✅ Reduz barreira de entrada (plano free)
- ✅ MRR previsível (planos pagos)
- ✅ Upsell natural (limites free)
- ✅ Segmentação de clientes

**Trade-off:**
- ❌ Complexidade adicional no código
- ❌ Precisa gerenciar limites/cotas
...
### Inspiração de UX
- **Ticket Sports:** referência de features (não de preço)
- **Sympla:** checkout e busca de eventos
- **Eventbrite:** dashboard e relatórios

### Competitors
- Ticket Sports (principal)
- Sympla (eventos gerais)
- InscriçõesOnline (pequeno)
- Tottem (básico)
...
:root {
  ...
  --gray: #6b7280;         /* Gray - textos secundários */
...
**1. Webhook não funcionando localmente**
```bash
...
```bash
# Verificar NEXT_PUBLIC_APP_URL está correto
# Verificar callback_url configurado no MP
```

**3. Pagamento não aparece no dashboard**
```bash
# Verificar webhook chegou (logs)
# Verificar paymentId salvo corretamente na inscrição
...
- **Produção:** https://okesports.com.br (futuro)

### Ferramentas
- **Figma:** [link do design]
- **Notion:** [link da documentação]
...
**Status:** Backend MVP dos fluxos principais implementado ✅  
**Ajustes pendentes:** detalhes de split payment (taxa 10%) e segurança do webhook (assinatura + idempotência)  
**Sprint Atual:** M3/M4 (Pagamentos + Webhook) – consolidação e hardening  
**Próxima Sprint:** Frontend (páginas públicas e experiência completa do atleta)

**Progresso Backend (implementado em código):**
- ✅ **M1** - Setup e Core Mínimo (Auth JWT + helpers de criptografia)
- ✅ **M1.2** - Autenticação organizador (login/register, middleware, refresh token)
- ✅ **M1.3** - Integração básica OAuth Mercado Pago (organizador)
- ✅ **M2** - CRUD de Eventos + Modalidades (com slug único, paginação e contadores)
- ✅ **M3 (parcial)** - Inscrição e cálculo de pricing (taxa plataforma, totais)
- ✅ **M4 (parcial)** - Webhook Mercado Pago para confirmação automática de pagamentos

**Total implementado (aprox.):**
- ✅ ~26 endpoints REST
- ✅ Autenticação e autorização para organizador e participante
- ✅ Sistema de pricing com lotes/cupons modelado em `prisma/schema.prisma`
- ✅ Integração Mercado Pago (OAuth + criação de preference + webhook básico)
- ✅ Validações Zod em grande parte das rotas
...
- Sempre usar TypeScript (não JavaScript puro)
- Preferir Server Components no Next.js (usar 'use client' apenas quando necessário)
...
- Idempotência em webhooks (não processar 2x)
- Usar sandbox do MP para testes

---

**Este arquivo é atualizado automaticamente pelo Claude Code durante o desenvolvimento.**

**Última sincronização:** 06/11/2025
